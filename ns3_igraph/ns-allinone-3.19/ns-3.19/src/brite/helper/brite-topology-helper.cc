...
void
BriteTopologyHelper::PrintBriteTopology ()
{
  std::ofstream f_topology;
  f_topology.open(m_topoFile.c_str(), std::ios_base::out | std::ios_base::trunc);
  f_topology << "Node ID, (X,Y), (inDegree, outDegree), AS ID, Type";
  for (BriteTopologyHelper::BriteNodeInfoList::iterator it = m_briteNodeInfoList.begin (); it != m_briteNodeInfoList.end (); ++it)
  {
     
     BriteNodeInfo n = *it;
     f_topology << n.nodeId << ",  (" << std::setw(5) << n.xCoordinate << ',' << std::setw(5) << n.yCoordinate << " ),  (" << std::setw(6) << n.inDegree << ", " << std::setw(6) << n.outDegree << "),   " << std::setw(2) << n.asId << ",   " << std::setw(20) << n.type; 
     f_topology << std::endl;
  }
  f_topology.close();
