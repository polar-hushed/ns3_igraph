...
// client_IP - IP corressponding to client interface that connects to this bridge
// client_MAC - MAC address of client interface that connects to this bridge
...
anycast_rtg::MF_AnycastRouting(MY_GUID $my_GUID, FORWARDING_TABLE rtg_tbl);
...
holdQ::ThreadSafeQueue(100); //delayed chunk Q for destns w/ poor link/path conditions
...
//Allow only IP tunneled MF pkts from edge interface (IP Protocol ID = 0x05)
//no LSA 34/00000005,    // TODO: block incoming LSAs?
fd_edge -> SetTimestamp -> edge_cla::Classifier(12/0800 23/05, -);       

edge_cla[1] -> Discard;

//Strip eth and IP hdrs and encap with a ethernet hdr with MF ethertype
edge_cla[0] 
	-> Strip(34) 
	-> EtherEncap(0x27C0, $client_MAC, $edge_dev)
	-> Paint(1, 16) 
	-> inQ
...
net_binder[0] 
	-> svc_cla::Classifier(
			...
			-);          // p3 Unhandled type, discard

svc_cla[0] -> intra_lkup; // default rtg
svc_cla[1] -> Print(WARN_MCAST_SVC_TYPE) ->  Discard;//multicast_rtg; // multicast
svc_cla[2] -> anycast_rtg; // anycast
svc_cla[3] -> Print(WARN_UNKNOWN_SVC_TYPE) ->  Discard; //TODO: handle other service types. Danger, chunks are packet containers! Need a chunk muncher.
...
//Forwarding decisions: anycast routing

anycast_rtg[0] -> segQ; //send chunk to next hop
// Hold if (1) dest not found in FT or (2) poor net condns to dest
anycast_rtg[1] -> holdQ; //no forwarding entry
anycast_rtg[2] -> holdQ; //hold decision
...
out_switch[1] 
	//strip eth hdr with MF ethertype and encap as IP 
	-> Strip(14)
	//Use IP protocol ID=5 for encapsulating MF pkts  
	-> IPEncap(5, $edge_dev_IP, $client_IP)
	//set ethertype to IP
	//TODO learn and lookup host mac automatically
	-> EtherEncap(0x800, $edge_dev, $client_MAC)
	-> outQ_edge -> td_edge;
